 ...
 *
 * Borrowed from https://github.com/lodash/lodash/blob/f7dc3b9ce4cbe6107c28d37f434ace12d0027a63/test/saucelabs.js
 */
...
var advisor = getOption('advisor', false),
    ...
    framework = getOption('framework', 'mocha'),
    idleTimeout = getOption('idleTimeout', 90),
    jobName = getOption('name', 'DOMtastic tests'),
    ...
    recordScreenshots = getOption('recordScreenshots', true),
...
var platforms = [
    ...
    ['OS X 10.11', 'iphone', '8.4'],
    ['OS X 10.11', 'iphone', '9.0'],
    ['OS X 10.10', 'ipad', '7.1'],
    ['OS X 10.10', 'ipad', '8.2'],
    ['OS X 10.11', 'ipad', '8.4'],
    ['OS X 10.11', 'ipad', '9.0'],
    ['OS X 10.10', 'chrome', '45'],
    ['OS X 10.10', 'firefox', '41'],
    ['OS X 10.10', 'safari', '8'],
    //['OS X 10.11', 'safari', '8.1'],
    ['OS X 10.11', 'chrome', 'dev'],
    ['Windows 8', 'internet explorer', '10'],
    ['Windows 10', 'chrome', '32'],
    ['Windows 10', 'chrome', '45'],
    ['Windows 10', 'firefox', '26'],
...
function onJobStatus(error, res, body) {
    ...
    var description = browserName(platform[1]) + ' ' + platform[2] + ' on ' + capitalizeWords(platform[0]),
        errored = !jobResult || !jobResult.passes || reError.test(message) || reError.test(jobStatus),
        failures = _.result(jobResult, 'failures'),
        label = options.name + ':',
        tunnel = this.tunnel;
    ...
    if (errored || failures) {
        ...
        logInline();
        if (failures) {
            console.error(label + ' %s FAILED %d test' + (failures > 1 ? 's' : '') + '. %s', description, failures, details);
        }
            ...
            console.error(label, description, 'FAILED;', message);
        }
    }
    else {
        logInline();
        console.log(label, description, 'passed');
    }
    this.running = false;
    this.emit('complete');
}
